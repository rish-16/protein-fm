...
from src.openfold.config import NUM_RES, NUM_EXTRA_SEQ, NUM_TEMPLATES, NUM_MSA_SEQ
from src.openfold.np import residue_constants as rc
...
def atom27_to_frames(na, eps=1e-8):
    ...
    all_atom_mask = na["all_atom_mask"]
    ...
    for restype, restype_letter in enumerate(nc.restypes):
        ...
        for torsion_idx in range(NUM_PROT_NA_TORSIONS):
            if nc.chi_angles_mask[resname][torsion_idx]:
                ...
                if (
                    names
                ):  # note: DNA molecules do not have `["N9"/"N1", "C1'", "C2'", "O2'"]` frames
